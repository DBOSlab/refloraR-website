...
Currently, **`r length(summary_df$collectionCode)`** collections are published on the platform. Among them, **`r length(summary_df$collectionCode[summary_df$Repatriated == TRUE])`** correspond to digitally repatriated collections, meaning sets of specimens collected in Brazil but physically housed in foreign herbaria, whose high-resolution images and associated metadata have been digitized by the holding institutions and made openly available through the REFLORA Virtual Herbarium. This “digital repatriation” thereby supports taxonomic revision, curation, and national biodiversity assessments in Brazil. In this context, “repatriation” refers to the return of access to information (images, label data, determinations, and related metadata) to Brazil via a public national infrastructure, rather than the physical return of specimens.
...
There are now more than 4,800,000 images of specimens available in the REFLORA Virtual Herbarium. Among them, 157,652 are nomenclatural types and 1,985,843 are georeferenced records.
